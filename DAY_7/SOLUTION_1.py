...
def compute(data, readPoint, phaseSignal, inputSignal):
    inputCount =0
    while(True):
        ...
        if (opcode == Opcodes.HALT):
            ...
            if inputCount == 0:
                data[data[readPoint+1]] = phaseSignal
            elif inputCount == 1:
                data[data[readPoint+1]] = inputSignal
            inputCount += 1

        elif (opcode == Opcodes.OUTPUT):
            #print(data[data[readPoint+1]])
            return data[data[readPoint+1]]
...
for a in range(5):
    data = copy.deepcopy(originalMemory)
    values[0] = compute(data, 0, a, 0)
    for b in range(5):
        if b == a:
            continue
        data = copy.deepcopy(originalMemory)
        values[1] = compute(data, 0, b, values[0])
        for c in range(5):
            if c == a or c == b:
                continue
            data = copy.deepcopy(originalMemory)
            values[2] = compute(data, 0, c, values[1])
            for d in range(5):
                if d == a or d == b or d == c:
                    continue
                data = copy.deepcopy(originalMemory)
                values[3] = compute(data, 0, d, values[2])
                for e in range(5):
                    if e == a or e == b or e == c or e ==d:
                        continue
                    data = copy.deepcopy(originalMemory)
                    values[4] = compute(data, 0, e, values[3])
                    print("Signal: {}, Phase: {}".format(sum(values), str(a)+str(b)+str(c)+str(d)+str(e)))
                    print(values)
                    bestOutput = max(bestOutput, values[4])
